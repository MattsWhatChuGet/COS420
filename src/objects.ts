...
export function makeBlankQuestion(
    ...
    return {body: "", expected: "", options: [], points: 1, published: false, id: id, name: name, type: type};
}
...
export function isCorrect(question: Question, answer: string): boolean {
    ...
    return answer == question.expected.toLowerCase();
}
...
export function isValid(question: Question, answer: string): boolean {
    if(question.type === "multiple_choice_question" && !question.options.includes(answer)){
        return false
    }
    return true;
}
...
export function toMarkdown(question: Question): string {
    ...
    if(question.type === "multiple_choice_question"){
        question.options.forEach((option) => format += "\n- " + option)
    }
    return format;
}
...
export function mergeQuestion(
    ...
    return {...contentQuestion, name: name, id: id, points: points, published: false};
}
